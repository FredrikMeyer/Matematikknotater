\chapter{Triangulations of spheres}
\label{sec:spheres8}
...
Unfortunaly, there seems to be a mistake in Grünbaums list. \todo{name which ones} Two of spheres listed have $H^3(K;k)=0$, which should not occur if they were spheres.
...
However, even in obstructed cases, we might have power series solutions, meaning that lifting the equations one step at a time will never terminate.
...
\todo{Regne ut $(T^1)^G$ og $(T^2)^G$}
\section{The five unobstructed triangulations}

There are five triangulations in Grünbaums list with $T^2=0$. 


\todo{compute - find smooth?? fiber}
%%%%% TODO: på datamaskin på blindern: sjekk om H^0(N) kan regnes ut!!!