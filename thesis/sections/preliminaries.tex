...
\end{proposition}
\begin{proof}
Let $S_X = \bigoplus_{d \geq 0} \OO_X(d)$ and $S_Y = \bigoplus_{d \geq 0} \OO_Y(d)$ be the homogeneous coordinate rings of $X$ and $Y$. Then $X \ast Y \subset \P^{n+m+1}$. 

Denote by $C(X \ast Y)$ the scheme $\Spec (S_X \otimes_k S_Y)$, the affine cone over $X \ast Y$. 

The singular locus of $C(X \ast Y) = C(X) \times C(Y)$ is equal to
\[
...
If the homogeneous coordinate ring of a projective variety $X$ is a Gorenstein ring, we say that $X$ is \emph{arithmetically Gorenstein}. In that case, the canonical sheaf can be computed as the sheaf associated to the graded module $\Ext^{\codim X}(S_X,S_X(-\dim N-1))=S_X(-d)$, for some $d$, where $N$ is the dimension of the linear system.
...
Parts of this thesis will be concerned with the construction of new Calabi--Yau manifolds.

\begin{definition}
A Calabi--Yau manifold will be an irreducible complex projective variety $X$ such that $\omega_X \simeq \OO_X$ and $H^i(X,\OO_X)=0$ for $i=1,\ldots,\dim X-1$.
\end{definition}
...
Recall that a variety is \emph{Fano} if the anticanonical line bundle $\omega_Y^{-1}$ gives an embedding into projective space. Given a Fano variety, one can construct a \CY variety: 
...
\]
Now note that all terms $\OO_Y(-j)$ with $0 < j < k$ are cohomologically trivial, in the sense that $H^\ast(Y,\OO_Y(-j))=0$. An induction argument then shows that $H^p(Y,\mathcal I_X) \simeq H^{p+k-1}(Y,\OO_Y(-k))$ for all $p$.
...
In many cases of interest, given a construction of a Calabi--Yau manifold, the following Ansatz produces a mirror. \todo{Spør Jan: er dette Janko Bohms konstruksjon i forkledning?}
...
\label{fig:p2fan}
\end{figure}
The spectra of these glue to form $\P^2$. More complicated fans give rise to exponents in the monomial generators.
\end{example}
